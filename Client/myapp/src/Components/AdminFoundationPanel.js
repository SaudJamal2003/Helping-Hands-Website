...
import AdminFoundationCard from './AdminFoundationCard';
import AdminFoundationTable from './AdminFoundationTable';
...
export default function AdminFoundationPanel() {
    const[foundationDetails, setDetails] = useState([]);
    const[totalDonations, settotalDonations] = useState(0);
    const[totalDonators, setTotalDonators] = useState(0);
    const[totalFoundations, settotalFoundations] = useState(null);

    const[queryName, setQueryName] = useState(null);
    const [foundationName, setFoundationName] = useState(null);
    ...
    const fetchData = () => {
        fetch('http://localhost:3002/api/adminFoundationPanel/foundationsDetails')
    ...
    const fetchData2 = () =>{
        fetch('http://localhost:3002/api/adminFoundationPanel/totalDonations')
        .then((response) => response.json())
        .then((data => {
    ...
    const fetchData3 = () => {
        fetch('http://localhost:3002/api/adminFoundationPanel/totalDonators')
        .then((response) => response.json())
        .then((data => {
    ...
    const fetchData4 = () => {
        fetch('http://localhost:3002/api/adminFoundationPanel/totalFoundations',{
            credentials: 'include'
        })
        .then((response) => response.json())
        .then((data => {
            console.log(data.count);
            settotalFoundations(data);
        }))
        .catch((error) => {
            console.log('Error', error);
            settotalFoundations({error:true});
        })
    }
    ...
    const handleSubmit = (e) => {
        e.preventDefault()
        fetch(`http://localhost:3002/api/adminFoundationPanel/foundationQueryRecord?queryName=${foundationName}`,{ 
        ...
        <div className={AdminCss.main}>
            <nav className={AdminCss.navbar}>
                <Link to='/'>
                    ...
                    <div className={AdminCss.mainMenu}>
                        <h4>MAIN MENU</h4>
                        {/* <Link to='/dashboard' > */}
            ...
            <div className={AdminCss.pageContent}>
                ...
                <div className={AdminCss.detailsDiv}>
                    ...
                    <div className={AdminCss.volunteerRequest}>
                        <div className={AdminCss.header}>
                            ...
                            <div className={AdminCss.dropdown}>
                                <button className={AdminCss.dropbtn}>Foundation Details</button>
                        ...
                        <div className={AdminCss.newVolunteers}>
                            <h5>Newly Registered Foundations</h5>
                            <div className={AdminCss.scrollBar}>
                                {foundationDetails.map((item) => {
                                            return<AdminFoundationCard foundation_name={item.foundation_name} foundation_id={item.foundation_id}
                                            email={item.email} location_id={item.location_id} _status={item._status}/>
                                        })}
                            </div> 
                        </div>
                        
                        <div className={AdminCss.searchVolunteer}>
                            <h5>Search Foundations</h5>
                            <form onSubmit={handleSubmit}>
                                <input type='text' placeholder='   Search Volunteers by foundation name'
                                onChange={handleInputChange} name='foundationName' value ={foundationName || ''} required/>
                                <button type='submit' onClick={handleSubmit} className={AdminCss.searchBtn}><svg xmlns="http://www.w3.org/2000/svg" width="30" height="30" viewBox="0 0 24 24" style={{fill: 'rgba(0, 0, 0, 1)'}}><path d="M10 18a7.952 7.952 0 0 0 4.897-1.688l4.396 4.396 1.414-1.414-4.396-4.396A7.952 7.952 0 0 0 18 10c0-4.411-3.589-8-8-8s-8 3.589-8 8 3.589 8 8 8zm0-14c3.309 0 6 2.691 6 6s-2.691 6-6 6-6-2.691-6-6 2.691-6 6-6z"></path></svg></button>
                            </form>
                        </div>
                    ...
                    <div className={AdminCss.countDiv}>
                        <div className={AdminCss.volunteerCount}>
                            <h4>Foundation Count</h4>
                            <h2 style={{color: '#FF7070'}}>{totalFoundations && totalFoundations.count}+</h2>
                        ...
                        <div className={AdminCss.totalDonation}>
                            ...
                            <h2 style={{color: '#FF7070'}}>{totalDonators && totalDonators.users}+</h2>
                            <img src={percentage} alt='percent' className={AdminCss.percent}/>
                ...
                <div className={AdminCss.table}>
                    ...
                    <p className={AdminCss.name}>{queryName.foundation_name}</p>
                    <p className={AdminCss.donationType}>{queryName.foundation_id}</p>
                    <p>{queryName.location_id}</p>
                    <p className={AdminCss.date}>{queryName.email}</p>
                    ...
                    <div id={AdminCss.tableScroll}  style={{ display: showTableScroll ? 'block' : 'none' }}>
                        <div className={AdminCss.scrollBar}>
                            {foundationDetails.map((item) => {
                            return <AdminFoundationTable foundation_name={item.foundation_name} foundation_id={item.foundation_id}
                            email={item.email} location_id={item.location_id} _status={item._status} />
                            })}
                        </div>
                    </div>
