...
export default function ForgotPassword() {
    ...
    const handleSubmit = (e) => {
        fetch('http://localhost:3002/api/user/forgot-password',{
            credentials: 'include',
            method: 'POST',
            headers: {
        ...
        <div className={forgotPasswordCss.main}>
            <form onSubmit={handleSubmit} className={forgotPasswordCss.mailForm}>
                <div className={forgotPasswordCss.innerDiv}>
                    ...
                    <p>Email</p>
                    <input onChange={handleInput} name='email' type='text' placeholder='   user.name@mail.com' className={forgotPasswordCss.emailInput} required/>
                </div>
